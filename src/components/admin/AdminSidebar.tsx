...
export default function AdminSidebar({ adminUser }: AdminSidebarProps) {
  ...
  const allNavigation = [
    ...
    },
    {
      ...
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 3l3.057-3 3.943 3 3.943-3L19 3l1 6.5-4 4.5v7l-4-2-4 2v-7l-4-4.5L5 3z" />
        </svg>
      ),
    },
    {
  ...
  const navigation = allNavigation.filter(item => item.allowedRoles.includes(role))
        ...
        <div className="p-4 border-t-2 border-gray-800">
          <div className="px-3 py-2 bg-gray-900 border-2 border-gray-700 text-center">
            ...
            <div className={`text-sm font-bold uppercase mt-1 ${
              role === 'admin' ? 'text-red-400' : role === 'manager' ? 'text-brand-primary' : 'text-gray-400'
            }`}>
              {role}
            </div>
          </div>
          {role === 'viewer' && (
            <div className="mt-2 px-3 py-1.5 bg-yellow-900/30 border border-yellow-700/50 text-center">
              <div className="text-[10px] text-yellow-500 uppercase tracking-wider font-bold">Read-only</div>
            </div>
          )}
        </div>
      </aside>
