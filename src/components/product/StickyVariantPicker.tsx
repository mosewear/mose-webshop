...
export default function StickyVariantPicker({
  ...
  return (
    <>
      ...
      {/* Mobile variant: visible after hero has scrolled out */}
      <div
...
function StickyContent({
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 md:px-6 py-2.5 md:py-3 flex items-center gap-2 md:gap-4">
      {/* Colour swatches */}
      {availableColors.length > 0 && (
        <div className="flex items-center gap-1.5 md:gap-2 shrink-0">
          ...
          <div className="flex gap-1.5 md:gap-2 max-w-[7rem] md:max-w-none overflow-x-auto scrollbar-hide">
              ...
              return (
                <button
                  ...
                  className={`relative shrink-0 transition-all border-2 ${
                    isSelected
                      ? 'border-brand-primary w-7 h-7 md:w-8 md:h-8'
                      : available
                      ? 'border-gray-300 hover:border-black w-6 h-6 md:w-7 md:h-7'
                      : 'border-gray-200 w-6 h-6 md:w-7 md:h-7 opacity-50 cursor-not-allowed'
                  }`}
      ...
      {availableColors.length > 0 && availableSizes.length > 0 && (
        <span className="hidden md:block h-8 w-px bg-gray-200 shrink-0" />
      )}

      {/* Size pills */}
      {availableSizes.length > 0 && (
        <div className="flex items-center gap-1.5 md:gap-2 shrink min-w-0 flex-1">
          {!isMobile && (
            <span className="text-[10px] font-bold uppercase tracking-wider text-gray-500 hidden lg:inline shrink-0">
              {tSizeLabel}
            </span>
          )}
          <div className="flex gap-1.5 md:gap-2 overflow-x-auto scrollbar-hide min-w-0">
            {availableSizes.map((size) => {
              ...
              return (
                <button
                  ...
                  disabled={!available}
                  className={`shrink-0 px-2.5 md:px-3 py-1.5 md:py-2 text-[11px] md:text-xs font-bold uppercase tracking-wider transition-all border-2 ${
                    isSelected
      ...
      {/* Price (desktop only - on mobile we hide it to save space; price is in the CTA) */}
      {!isMobile && (
      ...
      <button
        ...
        className={`shrink-0 inline-flex items-center justify-center gap-1.5 md:gap-2 px-3 md:px-5 py-2.5 md:py-3 text-xs md:text-sm font-bold uppercase tracking-wider transition-all border-2 ${
          ctaDisabled
