...
export default function AccountPage() {
  ...
  async function updateProfile() {
    ...
    try {
      ...
      toast.success(t('profileSection.saveSuccess'))
      await fetchProfile()
    } catch (error: any) {
      toast.error(error.message || t('profileSection.saveError'))
    } finally {
  ...
  async function updatePassword() {
    ...
    try {
      ...
      setPasswordError(error.message || t('passwordSection.saveError'))
    } finally {
  ...
  async function fetchAddresses() {
    ...
    try {
      ...
      if (error.code !== '42P01' && !error.message?.includes('does not exist')) {
        toast.error(t('addressSection.saveError'))
      }
      setAddresses([])
    } finally {
  ...
  async function saveAddress() {
    ...
    try {
      ...
      if (editingAddress) {
        ...
        if (error) throw error
        toast.success(t('addressSection.saveSuccess'))
      }
      ...
      setAddressError(error.message || t('addressSection.saveError'))
    }
  }

  async function deleteAddress(id: string) {
    if (!confirm(t('addressSection.deleteConfirm'))) return
    ...
    <div className="min-h-screen pt-6 md:pt-8 px-4 pb-16">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div>
            <h1 className="text-4xl md:text-5xl font-display mb-2">{t('title')}</h1>
        ...
        <div className="grid md:grid-cols-4 gap-6">
          ...
          <div className="md:col-span-3">
            ...
            {activeTab === 'orders' && (
              <>
                <h2 className="text-2xl md:text-3xl font-display mb-6">{t('myOrders')}</h2>

                {orders.length === 0 ? (
                  <div className="bg-gray-50 border-2 border-gray-300 p-8 md:p-12 text-center">
                    <p className="text-gray-600 mb-6 text-lg">{t('ordersEmpty')}</p>
                    <LocaleLink
                      href="/shop"
            ...
            {activeTab === 'profile' && (
              <div className="space-y-6">
                <h2 className="text-2xl md:text-3xl font-display mb-6">{t('profileSection.title')}</h2>

                {/* Profile Information */}
                <div className="bg-white border-2 border-black p-6 md:p-8">
                  <h3 className="text-xl font-display mb-4">{t('profileSection.title')}</h3>
                  <div className="space-y-4">
                    ...
                    >
                      {profileLoading ? t('profileSection.saving') : t('profileSection.save')}
                    </button>
                  </div>
                </div>
                ...
                {/* Password Change */}
                <div className="bg-white border-2 border-black p-6 md:p-8">
                  <h3 className="text-xl font-display mb-4">{t('passwordSection.title')}</h3>
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-bold mb-2">
                    ...
                    <button
                      ...
                      className="px-6 py-3 bg-brand-primary text-white font-bold uppercase text-sm hover:bg-brand-primary-hover transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {passwordLoading ? t('passwordSection.saving') : t('passwordSection.save')}
                    </button>
                  </div>
                </div>
              </div>
